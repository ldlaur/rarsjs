...
function compile(outpath, optimize) {
  ...
  return new Promise((resolve, reject) => {
    ...
    exec(
      `clang --target=wasm32 -flto -nostdlib -Wl,--export-all -Wl,--no-entry -Wl,--allow-undefined -Wl,--import-memory ${opts} -o ${outpath}/main.wasm src/exec/core.c src/exec/emulate.c src/exec/callsan.c src/exec/wasm.c`,
      (error, stdout, stderr) => {
        if (error) {
          reject(stderr);
        } else {
